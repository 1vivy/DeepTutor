...
import { setWatchingMaterial, setWatchingViewport } from '@/lib/watching-turn-state'
...
interface WatchingContextValue {
  ...
  error: string | null
  lastUrl: string
  openUrl(url: string, language?: string, providerOverride?: VideoProvider): Promise<void>
...
export function WatchingProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void getVideoMaterial(materialId)
      ...
      .catch(caught => {
        setError(
          caught instanceof Error ? caught.message : t('The player provider is unavailable.')
        )
  ...
  const openUrl = useCallback(
    async (url: string, language = '', providerOverride?: VideoProvider) => {
      ...
      try {
        ...
        setError(caught instanceof Error ? caught.message : t('This video could not be opened.'))
        throw caught
      } finally {
        setLoading(false)
      }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : t('The player provider is unavailable.'))
    } finally {
      setLoading(false)
    }
  }, [accept, material, t])
  ...
  const reportTime = useCallback((seconds: number) => setWatchingViewport(seconds), [])
  ...
  const value = useMemo(
    ...
    [material, active, loading, error, lastUrl, openUrl, refresh, close, reportTime, clearError]
  )
...
export function useWatching(): WatchingContextValue {
  ...
  if (!context) throw new Error('useWatching must be used inside WatchingProvider')
  return context
}
