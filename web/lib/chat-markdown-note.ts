...
const PENDING_CHAT_MARKDOWN_NOTE_KEY = `${CHAT_MARKDOWN_NOTE_KEY_PREFIX}pending`;
...
function storageKey(sessionId: string | null): string {
  return sessionId
    ? `${CHAT_MARKDOWN_NOTE_KEY_PREFIX}${sessionId}`
    : PENDING_CHAT_MARKDOWN_NOTE_KEY;
}
...
export function loadChatMarkdownNoteDraft(
  ...
  const pending = parseDraft(
    storage.getItem(PENDING_CHAT_MARKDOWN_NOTE_KEY),
  );
  if (!draft && pending) {
    try {
      storage.setItem(key, JSON.stringify(pending));
      storage.removeItem(PENDING_CHAT_MARKDOWN_NOTE_KEY);
    } catch {
