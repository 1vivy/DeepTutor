...
export const sessionClient: SessionClient = {
  get(sessionId, signal) {
    return requestJson<SessionWireDetail>(`/api/v1/sessions/${id(sessionId)}`, {
      cache: "no-store",
      signal,
      scope: "session",
    });
  ...
  remove(sessionId) {
    return requestVoid(`/api/v1/sessions/${id(sessionId)}`, {
      method: "DELETE",
      scope: "session",
    });
  },
  selectBranch(sessionId, selectedBranches) {
    return requestVoid(`/api/v1/sessions/${id(sessionId)}/branch-selection`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  removeMessage(sessionId, messageId) {
    return requestVoid(`/api/v1/sessions/${id(sessionId)}/messages/${messageId}`, {
      method: "DELETE",
      scope: "session",
    });
  },
