...
function unitKey(materialId: string, locator: number): string {
  return `${materialId}:${locator}`;
}
...
export default function ReadingReferencePicker({
      ...
      new Set(
        selected.flatMap((reference) =>
          reference.units.map((unit) =>
            unitKey(reference.materialId, unit.locator),
          ),
        ),
      ),
    [selected],
  ...
  const allActiveSelected =
    ...
    activeUnits.every((unit) =>
      selectedKeys.has(unitKey(activeMaterial!.material_id, unit.locator)),
    );

  const addUnits = (material: MaterialInfo, units: SelectedReadingUnit[]) => {
    const existing = selected.find(
      (reference) => reference.materialId === material.material_id,
    );
  ...
  const removeUnits = (materialId: string, locators: Set<number>) => {
    setSelected((previous) =>
      previous
        .map((reference) =>
  ...
  const toggleUnit = (unit: SelectedReadingUnit) => {
    ...
    if (selectedKeys.has(unitKey(activeMaterial.material_id, unit.locator))) {
      removeUnits(activeMaterial.material_id, new Set([unit.locator]));
      return;
    }
                ...
                <div className="divide-y divide-[var(--border)] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--background)]/35">
                  {activeUnits.map((unit) => {
                    const checked = selectedKeys.has(
                      unitKey(activeMaterial.material_id, unit.locator),
                    );
                    return (
                      <button
                        type="button"
