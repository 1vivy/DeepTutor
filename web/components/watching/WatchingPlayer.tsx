...
function YouTubePlayer({
  ...
  useEffect(() => {
    ...
    void loadYouTubeApi()
          ...
          events: {
            onReady: event => {
              ...
              timer = window.setInterval(
                () => onTime(controller?.currentTime() || 0, controller?.duration() || 0),
                250
              )
            },
            onStateChange: event => {
      ...
      .catch(caught => {
        if (!cancelled) {
          onError(caught instanceof Error ? t(caught.message) : t('YouTube playback failed.'))
        }
      })
    return () => {
      cancelled = true
  ...
  }, [onController, onError, onPersist, onTime, playback.start_seconds, playback.video_id, t])
...
function InvidiousPlayer({
  ...
  useEffect(() => {
    ...
    onController(controller)
    const report = () => onTime(controller.currentTime(), controller.duration())
    const ready = () => {
      if (playback.start_seconds > 0) controller.seek(playback.start_seconds)
      report()
    }
  ...
  return (
    <video
      ...
      onError={() => onError(t('The configured Invidious stream could not be played.'))}
    >
      <track
        kind="subtitles"
