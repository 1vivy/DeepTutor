...
export default function VideoLearningSettingsPage() {
  ...
  useEffect(() => {
    void getVideoLearningSettings()
      .then(setSettings)
      .catch(error =>
        setMessage(error instanceof Error ? error.message : t('Settings could not be loaded.'))
      )
  ...
  const save = async () => {
    ...
    try {
      ...
      setMessage(t('Saved. New and reopened videos use this provider immediately.'))
    } catch (error) {
      setMessage(error instanceof Error ? error.message : t('Settings could not be saved.'))
    } finally {
      setSaving(false)
    }
  }
  ...
  const test = async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : t('Connection test failed.'))
    }
  }
      ...
      {loading ? (
        ...
        <>
            ...
            {(
              [
                [
                  'youtube',
                  Youtube,
                  t('Official privacy-enhanced IFrame Player. No media is proxied or stored.'),
                ],
                [
                  'invidious',
                  Server,
                  t('HTML5 playback through your configured Invidious instance.'),
                ],
              ] as const
              ...
              >
                <div className="flex items-center gap-2 font-medium">
                  ...
                  {provider === 'youtube' ? t('Native YouTube') : t('Self-hosted Invidious')}
                </div>
          ...
          <div className="space-y-4 rounded-xl border border-[var(--border)] p-5">
            ...
            <label className="block text-sm">
              <span className="mb-1 block">{t('Browser-facing origin (optional)')}</span>
              <input
            ...
            <select
              ...
              onChange={event =>
                setSettings(current => ({
                  ...current,
                  youtube: {
                    transcript_provider: event.target.value as 'youtube_transcript_api' | 'none',
                  },
                }))
              }
              className="rounded-lg border border-[var(--border)] bg-transparent px-3 py-2"
            >
              <option value="youtube_transcript_api">{t('youtube-transcript-api')}</option>
