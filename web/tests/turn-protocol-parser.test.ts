...
test("active turn information is validated and heartbeats are quarantined", () => {
  ...
  for (const heartbeat of ["", "heartbeat", '{"type":"ping"}', '{"type":"pong"}']) {
...
test("invalid, incomplete, future, and version-mismatched frames are rejected", () => {
  const missing = parseTurnEvent({ type: "content", turn_id: "turn-1", timestamp: 1 });
...
test("command builders enforce IDs, sequence bounds, replies, and v2 versioning", () => {
  ...
  assert.equal(buildCancelTurn("turn-1").type, "cancel_turn");
  assert.equal(buildSubmitUserReply({ turnId: "turn-1", text: "yes" }).text, "yes");
