...
test("markdown-note drafts persist and restore by chat session", () => {
  ...
  assert.deepEqual(loadChatMarkdownNoteDraft("session-1", noteStorage), {
    title: "Session note",
    content: "# Session content",
    saved: { id: "doc-1", title: "Session note", content: "# Session" },
  });
  assert.deepEqual(loadChatMarkdownNoteDraft("session-2", noteStorage), {
    title: "",
    content: "",
    saved: null,
  });
...
test("a pending markdown-note draft is adopted by its chat session", () => {
  ...
  assert.equal(storage.has("dt:chat-markdown-note:pending"), false);
  assert.equal(
    storage.get("dt:chat-markdown-note:session-1"),
    JSON.stringify({
      title: "Pending note",
      content: "# Pending",
      saved: null,
...
): Parameters<typeof saveChatMarkdownNoteDraft>[2] {
  return {
    getItem(key) {
