...
test("cancel stays connected and a waiting-input reply survives only unacknowledged reconnects", () => {
  const { client, scheduler, sockets } = harness();
  client.setResumeCursor("turn-1", 2);
  client.connect();
  sockets[0].open();
  client.cancel(buildCancelTurn("turn-1"));
  client.send(buildSubmitUserReply({ turnId: "turn-1", text: "continue" }));
  ...
  scheduler.runNext();
  sockets[2].open();
  assert.equal(sockets[2].sent.filter((item) => item.type === "submit_user_reply").length, 0);
});
