...
def test_reference_normalization_rejects_paths_and_deduplicates_locators() -> None:
    assert normalize_reading_references(
        [
            ...
            {"material_id": "ABCDEF0123456789", "locators": [2, 2, "3", 0, True, 1.5]},
            {"material_id": "abcdef0123456789", "locators": [4]},
        ]
    ) == [{"material_id": "abcdef0123456789", "locators": [2, 3, 4]}]


def test_reference_normalization_bounds_the_total_unit_count() -> None:
    refs = normalize_reading_references(
        [{"material_id": "abcdef0123456789", "locators": list(range(1, 100))}]
    )
...
def test_empty_rows_do_not_consume_the_material_limit() -> None:
    empty_rows = [{"material_id": f"{index:016x}", "locators": []} for index in range(20)]

    assert normalize_reading_references(
        [*empty_rows, {"material_id": "abcdef0123456789", "locators": [1]}]
...
def test_references_resolve_from_the_store_not_client_text(tmp_path: Path) -> None:
    ...
    assert resolved[0].source_id == f"rd-{manifest.material_id}-1"
