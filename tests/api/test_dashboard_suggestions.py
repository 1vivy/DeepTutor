...
def test_suggestions_is_not_swallowed_by_the_entry_route(
    ...
    async def _get(language: str = "en") -> dict[str, Any]:
        return {
    ...
    response = client.get("/api/v1/dashboard/suggestions", params={"language": "zh"})
...
def test_refresh_returns_a_fresh_set(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _refresh(language: str = "en") -> SuggestionSet:
