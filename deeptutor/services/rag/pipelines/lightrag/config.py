...
def lightrag_llm_selection_from_settings() -> dict[str, str] | None:
    """Read the stable catalog reference for LightRAG's dedicated LLM.

    Both IDs are required so callers cannot accidentally combine one half of an
    old selection with the currently-active half of another model. Empty fields
    mean "use the global active chat model".
    """
    try:
        from deeptutor.services.config import load_lightrag_settings
        ...
        logger.warning(
            "Could not read LightRAG LLM selection; using the active model", exc_info=True
        )
        return None


def _resolve_override_llm_config(selection: dict[str, str] | None):
    """Resolve a stored catalog selection without exposing catalog secrets."""
...
def build_llm_model_func(
    *,
    ...
    """
    config = _resolve_override_llm_config(llm_selection)
    if config is None:
        ...
        base = LLMClient(config=config, configure_env=False).get_model_func()
...
def build_vision_model_func(
    *,
    ...
    if config is None:
        ...
        base = LLMClient(config=config, configure_env=False).get_vision_model_func()
