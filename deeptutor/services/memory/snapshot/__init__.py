...
def read_stamps(surface: Surface) -> list[EntityStamp]:
    """Diff-shaped view of a surface (no content). See :mod:`.adapters`."""
...
def refresh_snapshot(surface: Surface) -> list[ChangeEntry]:
    ...
    and labels, so reading message bodies here would be pure waste — and this
    path is what an automatic refresh calls after every turn.
    """
