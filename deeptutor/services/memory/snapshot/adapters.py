...
# ── Probes (fingerprints without content) ────────────────────────────
...
def probe_chat_entities() -> list[EntityStamp]:
    ...
    Skipping ``content`` is what makes an automatic per-turn refresh viable:
    the full read concatenates every message of every session (~7 MB here).
    """
    db_path = get_path_service().get_chat_history_db()
    if not db_path.exists():
        return []
...
# skipping content actually saves something: every other adapter reads well
# under a megabyte, so a second code path would be duplication without a
# payoff — and a second chance to drift from its full reader.
_PROBES = {
...
def read_stamps(surface: Surface) -> list[EntityStamp]:
    """Identity + label + fingerprint for every entity on *surface*.
